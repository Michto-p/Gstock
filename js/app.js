// js/app.js — v1.8.0 : Modale Ajout/Retrait après scan + réassort + indicateur + prêts
(function(){
  ...
  // ===== Scanner (stock) + MODALE =====
  const video = document.getElementById('video');
  const scanStatus = document.getElementById('scanStatus');
  const qtyDefaultInput = document.getElementById('qty');
  ...
  // Modale éléments
  const modalBackdrop = document.getElementById('qtyModalBackdrop');
  const modalInfo = document.getElementById('qtyModalInfo');
  const modalName = document.getElementById('qtyModalName');
  const modalQty = document.getElementById('qtyModalQty');
  const modalOk = document.getElementById('qtyModalOk');
  const modalCancel = document.getElementById('qtyModalCancel');

  let modalModeDefault = 'out'; // valeur pré-cochée (suivra le dernier choix)
  function setModalMode(val){
    const radios = document.querySelectorAll('input[name="qtyModalMode"]');
    radios.forEach(r=> r.checked = (r.value===val));
  }
  function getModalMode(){
    const r = document.querySelector('input[name="qtyModalMode"]:checked');
  ...
  let scanning = false, lastDetected = '', lastBeepAt = 0, modalOpen = false;
  ...
  if (btnTestDetect) btnTestDetect.addEventListener('click', async ()=>{
    ...
    if (v){ if (v!==lastDetected){ lastDetected=v; beepOK(); } if (lastOp) lastOp.innerHTML = '🔎 Détecté : <b>'+v+'</b>'; openQtyModal(v); }
  ...
  async function loopScan(){
    while(scanning){
      ...
      try{
        ...
        if (modalOpen) continue; // pause la détection pendant la modale
  ...
  function showModal(){ if (modalBackdrop){ modalBackdrop.style.display='flex'; modalOpen=true; } }
  function hideModal(){ if (modalBackdrop){ modalBackdrop.style.display='none'; modalOpen=false; } }

  async function openQtyModal(code){
    // Remplir infos
    if (modalInfo) modalInfo.textContent = 'Code : ' + code;
    let it = await dbGet(code);
    if (!it){
      // Pré-crée une fiche "fantôme" qui sera finalisée à la validation
      it = { barcode: code, name: 'Article ' + code, qty: 0, min: 0, tags: [], createdAt: Date.now(), updatedAt: Date.now() };
    }
    if (modalName) modalName.value = it.name || '';
    if (modalQty) modalQty.value = Math.max(1, parseInt(qtyDefaultInput && qtyDefaultInput.value || '1',10));
    setModalMode(modalModeDefault || mode); // coche le dernier choix ou le mode actuel
    showModal();

    // Boutons
    const onCancel = ()=>{ hideModal(); cleanup(); };
    const onOk = async ()=>{
      const q = Math.max(1, parseInt(modalQty && modalQty.value || '1',10));
      const chosen = getModalMode(); modalModeDefault = chosen;
      // créer/mettre à jour l’article si nécessaire
      it.name = (modalName && modalName.value ? modalName.value.trim() : it.name);
      const exists = !!(await dbGet(code));
      if (!exists) await dbPut(it);

      const delta = (chosen==='in') ? q : -q;
      const updated = await dbAdjustQty(code, delta, { mode: chosen, source:'modal' });
      scheduleFileSave();

      if (lastOp){
        const warn = getStatus(updated).level!=='ok';
        lastOp.innerHTML = (chosen==='in'?'✅ Entrée':'📤 Sortie') + ' <b>'+q+
          '</b> × <b>'+ (updated.name||'') + '</b> (<code>'+ updated.barcode +
          '</code>) — stock: <span class="'+(warn?'warntext':'oktext')+'">'+ updated.qty +'</span>';
      }
    ...
    if (modalOk) modalOk.addEventListener('click', onOk);
  ...
  async function refreshLoansTable(){
    ...
    let loans = await dbListLoans(true);
  ...
  // ===== Paramètres : presets + fichier (si activé côté app)
  ...
  function scheduleFileSave(){
    ...
    if (!auto || !auto.checked || !fileHandle) return;
