...
/* Démarrage caméra (HD + zoom/AF si possible) */
async function startCamera(videoEl){
  ...
  const missing=want.filter(f=>!supported.includes(f));
  if(missing.length && !window.__gstock_warnedFormats){
    alert('⚠️ Votre appareil ne signale pas le support natif de: '+missing.join(', ')+'\nEssayez un autre appareil, ou utilisez des QR codes.');
    window.__gstock_warnedFormats=true;
  }
...
/* Détection multi-voies (renforcement contraste) */
async function detectAny(videoEl){
  ...
  try{
    ...
    try{ const img=offCtx.getImageData(0,0,targetW,targetH); const d=img.data; const f=1.2;
...
/* Boucle principale — retourne un code CONNU uniquement */
      ...
      try{
        ...
        if(results&&results[0]){ const raw=results[0].rawValue||''; if(raw){
          ...
          if(!inCooldown && candidateCount>=confirmFrames){
            let known=null; try{ known=await dbGet(raw); }catch(_){}
