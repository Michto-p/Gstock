/* Gstock - barcode.js v2.1.5 (scan robuste) */
'use strict';
...
async function startCamera(videoEl){
  ...
  try{ mediaStream=await tryConstraints({ video:{ facingMode:{ideal:'environment'}, width:{ideal:1920}, height:{ideal:1080}, focusMode:'continuous' }, audio:false }); }
  catch(_){ const id=await pickRearCameraDeviceId(); if(id){ try{ mediaStream=await tryConstraints({ video:{ deviceId:{exact:id}, width:{ideal:1920}, height:{ideal:1080} }, audio:false }); }catch(_2){} }
...
async function scanUntilKnown(videoEl,{confirmFrames=2,cooldownMs=700,maxMs=60000}={}){
  await startCamera(videoEl).catch(()=>null);
  ...
  async function lookup(code){
    let it=await dbGet(code); if(it)return it;
    it=await dbGet(code.toUpperCase()); if(it)return it;
    it=await dbGet(code.toLowerCase()); if(it)return it;
    return null;
  }
  return new Promise(resolve=>{
    ...
    const tick=async()=>{
      ...
      if('requestVideoFrameCallback' in videoTrack?.__proto__?CURRENT_VIDEO_EL:{}){ CURRENT_VIDEO_EL.requestVideoFrameCallback(()=>{ tick(); }); }
      else{ requestAnimationFrame(tick); }
    };
    if('requestVideoFrameCallback' in CURRENT_VIDEO_EL){ CURRENT_VIDEO_EL.requestVideoFrameCallback(()=>tick()); } else { requestAnimationFrame(tick); }
  });
}
